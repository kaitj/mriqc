#!/usr/bin/env python
# -*- coding: utf-8 -*-
# emacs: -*- mode: python; py-indent-offset: 4; indent-tabs-mode: il -*-
...
class ExtractB0(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        imgs = np.array(nb.four_to_three(nb.load(self.inputs.in_dwi,
                                            mmap=NUMPY_MMAP)))
        bval = np.loadtxt(self.inputs.in_bval)
        max_b = 10.0
        
        index = np.argwhere(bval <= max_b).flaten().tolist()
        ...
        for b0 in b0s[1:]:
            ...
            nb.Nifti1Image(b0, imgs[0].afine, hdr).to_filename(
                filename[count-1])
            count += 1
